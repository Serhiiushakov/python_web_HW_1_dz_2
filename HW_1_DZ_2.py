...
def parse_input(user_input):
    if not user_input:  ## Обробка пустого вводу команди в боті
        return None, []
...
#03.06.2024## Створення абстрактного базового класу для користувацьких уявлень
class UserView(ABC):
    ...
    def run(self):
        self.display_message("Welcome to the assistant bot!")
        while True:
            user_input = self.get_user_input("Enter a command: ").strip().lower()
            command, args = parse_input(user_input)
            
            if command in ["close", "exit"]:
                save_data(self.book)
                self.display_message("Good bye!")
                break

            elif command == "hello":
                self.display_message("How can I help you?")

            elif command == "add":
                self.display_message(add_contact(args, self.book))

            elif command == "change":
                self.display_message(change_contact(args, self.book))

            elif command == "phone":
                self.display_message(show_phone(args, self.book))

            elif command == "all":
                self.display_message(show_all(self.book))

            elif command == "add-birthday":
                self.display_message(add_birthday(args, self.book))

            elif command == "show-birthday":
                self.display_message(show_birthday(args, self.book))

            elif command == "birthdays":
                self.display_message(birthdays(self.book))

            elif command is None:  ## Обробка пустого вводу
                self.display_message("Please enter a command.")

            else:
                self.display_message("Invalid command.")

#03.06.2024## Реалізація користувацького уявлення для консольного інтерфейсу
...
# Головна функція для запуску програми з консольним інтерфейсом
def main():
    book = load_data()
    view = ConsoleView(book)
    view.run()
